...
| Stage 3 | Structured Record and Query Boundary Definition | Closed | Definition completed for text-first records and controlled queries; no implementation. |

Stage 3 is closed. The project is currently between stages, and no next stage has been defined.
...
* A complete text-first card record boundary with original `.md` cards as the source of truth.
* Markdown `body_text` preservation and `raw_yaml_json` preservation for future derived records.
* Image exclusion from Agent-facing fields.
* A structured database-first and controlled query boundary.

Detailed record:

* [`docs-dev/v0/stage-3-structured-record-query-boundary.md`](docs-dev/v0/stage-3-structured-record-query-boundary.md)
...
The next stage should be defined separately after Stage 3 closeout.

Do not assume implementation should begin automatically. No next stage is defined or named in this document.
...
* Stage 3 closed after its documentation-only product-definition decisions were approved; no implementation was added.
